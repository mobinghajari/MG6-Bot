...
{
    STATE_SEARCHING_SPIKE,          // در جستجوی اسپایک
    STATE_MONITORING_CHANNEL,       // نظارت بر تشکیل کانال پس از اسپایک
    STATE_WAITING_FOR_ENTRY_TRIGGER // منتظر پولبک برای ورود
};
ENUM_STRATEGY_STATE strategyState = STATE_SEARCHING_SPIKE;
...
input int             InpAtr_Period             = 14;     // دوره اندیکاتور ATR
input int             InpSpike_Min_Candles      = 3;      // حداقل تعداد کندل برای اسپایک
input double          InpSpike_Body_Atr_Mult    = 1.3;    // ضریب ATR برای قدرت بدنه اسپایک
...
input int             InpOrder_Expiry_Candles   = 2;      // انقضای سفارش پس از چند کندل
...
bool CheckSessionFilters()
{
    ...
    if(dailyLossCount >= InpMax_Daily_Losses)
    {
        Print("Trading stopped for today: Maximum daily loss limit (", InpMax_Daily_Losses, ") reached.");
        return false;
    }
    ...
    if(dailyProfitPct >= InpMax_Daily_Profit_Percent)
    {
        Print("Trading stopped for today: Maximum daily profit target (", InpMax_Daily_Profit_Percent, "%) reached.");
        return false;
    }
    ...
    if(weeklyProfitPct >= InpMax_Weekly_Profit_Percent)
    {
        Print("Trading stopped for the week: Maximum weekly profit target (", InpMax_Weekly_Profit_Percent, "%) reached.");
        return false;
    }
...
void OnTick()
{
    ...
    {
        case STATE_SEARCHING_SPIKE:
            DetectSpike();
            break;
            
        case STATE_MONITORING_CHANNEL:
            DetectChannelAndBreakout();
            break;
            
        case STATE_WAITING_FOR_ENTRY_TRIGGER:
            // این وضعیت به طور مستقیم در اینجا مدیریت نمی‌شود
            // پس از شکست کانال، منتظر سفارش لیمیت می‌مانیم
            // اگر معامله‌ای انجام نشد، به حالت اولیه بازمی‌گردیم
            ResetStrategyState();
            break;
    }
}
...
void DetectSpike()
{
    ...
    MqlRates rates[];
    if(CopyRates(_Symbol, _Period, 0, InpSpike_Min_Candles + 20, rates) < InpSpike_Min_Candles + 20) return;
    ...
    double atr_values[];
    if(CopyBuffer(iATR(_Symbol, _Period, InpAtr_Period), 0, 0, InpSpike_Min_Candles + 20, atr_values) <= 0) return;
    ...
    for(int i = 1; i <= InpSpike_Min_Candles; i++)
    {
        ...
        if( !(body > 0 && body >= InpSpike_Body_Atr_Mult * atr_values[i] && rates[i].close > ema_values[i]) )
        {
            isBullSpike = false;
            break;
        }
    ...
    if(isBullSpike)
    {
        spikeDirection = 1;
        spikeEndBarIndex = 1;
        strategyState = STATE_MONITORING_CHANNEL;
...
void DetectChannelAndBreakout()
{
    ...
    // اگر بیش از 15 کندل از اسپایک گذشت و کانالی تشکیل نشد، استراتژی را ریست کن
    if(rates[0].time - rates[spikeEndBarIndex].time > 15 * PeriodSeconds())
    {
        Print("Channel did not form within 15 candles. Resetting strategy.");
        ResetStrategyState();
        return;
    }
...
void PlaceTradeOrder()
{
    ...
    {
        stopLoss = channelLastSwing - _Point * 5; // SL کمی پایین‌تر از آخرین کف کانال
        takeProfit = entryPrice + (entryPrice - stopLoss) * InpTake_Profit_RR_Ratio;
    }
    else // فروش
    {
        stopLoss = channelLastSwing + _Point * 5; // SL کمی بالاتر از آخرین سقف کانال
    ...
    if(OrderSend(request, result))
    {
        if(result.retcode == TRADE_RETCODE_DONE || result.retcode == TRADE_RETCODE_PLACED)
        {
            ...
            strategyState = STATE_WAITING_FOR_ENTRY_TRIGGER;
...
{
    strategyState = STATE_SEARCHING_SPIKE;
    spikeDirection = 0;
    spikeEndBarIndex = -1;
    channelLastSwing = 0;
    breakoutBarIndex = -1;
